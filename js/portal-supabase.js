...
let me=null, profile=null, services=[], therapists=[];
...
async function init(){
  ...
  $('#signup-form').onsubmit=async e=>{
    ...
    const d=formObj(e.target);
    const {data,error}=await sb.auth.signUp({
      email:String(d.email||'').trim().toLowerCase(),
      password:String(d.password||''),
      options:{data:{full_name:String(d.name||'').trim(),phone:String(d.phone||'').trim()}}
    });
    if(error)return msg(authMessage,error.message);
    if(!data.session){
      msg(authMessage,'Account created. Check your email and confirm your address, then sign in.','good');
      e.target.reset();
      return;
    }
    location.reload();
  ...
  if(['admin','super_admin'].includes(profile.role)){
    const l=$('#staff-link'); l.classList.remove('hidden'); l.href='admin.html'; l.textContent='Admin dashboard';
  } else if(profile.role==='therapist'){
    const l=$('#staff-link'); l.classList.remove('hidden'); l.href='therapist.html'; l.textContent='Therapist portal';
  }
  ...
  $('#book-therapist').onchange=loadSlots;
  $('#book-date').onchange=loadSlots;
  $('#booking-form').onsubmit=bookAppointment;

  const today=new Date(); today.setMinutes(today.getMinutes()-today.getTimezoneOffset());
  $('#book-date').min=today.toISOString().slice(0,10);
...
async function savePatient(e){
  ...
  if(error)return msg($('#portal-message'),error.message);
  e.target.reset(); $('#patient-form-wrap').classList.add('hidden');
  await loadPatients(); msg($('#portal-message'),'Profile saved.','good');
}
...
async function populateTherapists(){
  const sid=Number($('#book-service').value);
  $('#book-therapist').innerHTML='<option value="">Any available therapist</option>';
  if(!sid)return;
  const {data,error}=await sb.from('therapist_services').select('therapist_id,therapists(id,full_name,title)').eq('service_id',sid);
  if(error)return;
...
async function loadSlots(){
  const sid=Number($('#book-service').value), date=$('#book-date').value;
  const tid=$('#book-therapist').value?Number($('#book-therapist').value):null;
  $('#book-start').value='';
  const grid=$('#slot-grid');
  if(!sid||!date){grid.innerHTML=''; return;}
  ...
  const slots=data||[];
  $('#slot-help').textContent=slots.length?'Choose one available time.':'No open slots are currently configured for this date.';
  ...
  $$('.slot',grid).forEach(b=>b.onclick=()=>{
    $$('.slot',grid).forEach(x=>x.classList.remove('selected'));
    b.classList.add('selected');
    $('#book-start').value=b.dataset.start;
    $('#book-therapist').value=b.dataset.therapist;
  });
}
...
async function bookAppointment(e){
  e.preventDefault();
  const d=formObj(e.target);
  if(!d.start)return msg($('#portal-message'),'Please choose an available slot.');
  if(!d.therapist_id)return msg($('#portal-message'),'Choose a therapist or select an available slot.');
  const {error}=await sb.rpc('book_appointment',{
    p_patient_id:Number(d.patient_id),
    p_service_id:Number(d.service_id),
    p_therapist_id:Number(d.therapist_id),
    p_date:d.date,
    p_start:d.start,
    p_mode:d.mode||'clinic',
    p_notes:d.notes||null
  });
  if(error)return msg($('#portal-message'),error.message);
  msg($('#portal-message'),'Appointment requested. Ephphatha can now review and confirm it.','good');
  $('#book-start').value='';
