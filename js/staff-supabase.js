(()=>{
  ...
  const waitFor=(test,timeout=8000)=>new Promise((resolve,reject)=>{
    const start=Date.now();
    const tick=()=>{
      if(test()) return resolve();
      if(Date.now()-start>timeout) return reject(new Error('Ephphatha Supabase admin runtime did not initialize.'));
      setTimeout(tick,40);
    };
    tick();
  });
  (async()=>{
    try{
      ...
      if(target) target.innerHTML='<div class=\"notice\">'+String(err.message||err)+'</div>';
