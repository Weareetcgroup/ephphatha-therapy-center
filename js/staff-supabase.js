...
async function requireRole(roles){
  ...
  if(error||!user){location.replace('portal.html');return null;}
  const r=await sb.from('profiles').select('*').eq('id',user.id).single();
  if(r.error||!r.data||!roles.includes(r.data.role)){location.replace('portal.html');return null;}
  profile=r.data; return profile;
}
